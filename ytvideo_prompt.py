...
class YouTubePlayer:
    ...
    def run(self):
        while True:
            search_query = prompt("Enter a YouTube search query (or 'q' to quit): ")
            if search_query.lower() == 'q':
                break
            self.results = self.search_videos(search_query)
            self.print_search_results()
            video_num = int(prompt("Enter the number of the video you want to play: ")) - 1
...
if __name__ == "__main__":
    ...
    if not api_key:
        ...
        exit(1)
    player = prompt("Enter your preferred player (mpv/vlc): ", completer=WordCompleter(['mpv', 'vlc']))
