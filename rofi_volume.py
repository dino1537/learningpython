...
def show_volume_slider():
    ...
    volume_slider = ttk.Scale(volume_frame, from_=0, to=100, orient='horizontal', length=200, style='Horizontal.TScale')
    volume_slider.set(current_volume)
    volume_slider.grid(row=1, column=0, columnspan=2)

    ok_button = ttk.Button(volume_frame, text='OK', command=set_and_close, style='TButton')
    ok_button.grid(row=2, column=0, pady=10)

    cancel_button = ttk.Button(volume_frame, text='Cancel', command=root.destroy, style='TButton')
